#!/usr/bin/env bash
# Ubuntu baseline optimization (GNOME) — bash or zsh compatible

# If invoked via zsh, force sh-like semantics for compatibility.
...
# Log everything (stdout+stderr) to file + console
exec > >(tee -a "${LOG_FILE}") 2>&1

say() { printf '%s\n' "$*"; }

say "=== Ubuntu baseline optimization start (GNOME) ==="
say "Time: $(date -Is)"
...
say
say "[F] Restore GNOME monitor layout (Wayland)..."
...
if [ -f /etc/default/zramswap ]; then
  # One deterministic backup per run.
...
if command -v snap >/dev/null 2>&1; then
  # Remove everything except snapd; repeat passes to handle dependencies.
...
say
say "[T] GNOME responsiveness: disable ubuntu-report + tracker indexing..."

# ubuntu-report (user service)
systemctl --user mask ubuntu-report.path ubuntu-report.service 2>/dev/null || true

# tracker file indexer (user service) — Ubuntu GNOME commonly uses tracker3 units
